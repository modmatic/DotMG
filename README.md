...
The **DotMG** library is a fork of the [Arduboy2 library](https://github.com/MLXXXp/Arduboy2), with modifications made to allow compatibility with the Modmatic dotMG DIY handheld game console.
...
You must then create an DotMG class object:
...
The DotMG library reserves an area at the start of EEPROM for storing system information, such as the current audio mute state and the Unit Name and Unit ID. A sketch **must not** use this reserved area for its own purposes. A sketch may use any EEPROM past this reserved area. The first EEPROM address available for sketch use is given as the defined value *EEPROM_STORAGE_SPACE_START*

### Audio control functions

The library includes an DotMGAudio class. This class provides functions to enable and disable (mute) sound and also save the current mute state so that it remains in effect over power cycles and after loading a different sketch. It doesn't contain anything to actually produce sound.

The DotMGBase class, and thus the DotMG class, creates an DotMGAudio class object named *audio*, so a sketch doesn't need to create its own DotMGAudio object.
...
The *BeepPin1* and *BeepPin2* classes are available from the [*DotMGBeep*](https://github.com/modmatic/DotMGBeep) library to generate simple square wave tones.
